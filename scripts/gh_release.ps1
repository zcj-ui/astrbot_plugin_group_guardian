# gh_release.ps1 - create GitHub release + upload zip asset via direct IP (api.github.com)
param(
...
# create release
$createOut = & curl.exe -sk -X POST "$api/releases" `
...
$uploadOut = & curl.exe -sk -X POST "$api/releases/$releaseId/assets?name=$assetName" `
  -H "Host: api.github.com" -H "Authorization: Bearer $pat" `
