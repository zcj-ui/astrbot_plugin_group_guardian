...
20. **垃圾 Issue 标准流程**：纯推广/无建设性 Issue 直接分类 `other`，标记 `invalid` 并关闭，不进行可行性分析；若同一用户反复提交，建议启用自动化限制规则。无重复时也应声明检索范围以增强可审计性。

21. **文档醒目性反馈**：功能已存/文档入口类 Issue 连续出现时，主动提示文档交互设计改进（如 README 顶部加 FAQ 或显眼指令列表），从根源减少同类咨询。

22. **分类-标签一致性**：分类必须与推荐动作和标签对齐，避免 `other` 分类 + `invalid` 标签的错位；若仓库支持 `invalid`/`spam` 分类应直接使用，而非宽泛兜底。
