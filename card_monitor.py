...
# 店铺/推广链接特征：命中即视为违规名片，直接还原（不经 LLM）
_SHOP_LINK_RE = re.compile(
    r"(?:https?://|www\.)|"                                  # 任意网址
    r"(?:taobao|tmall|jd|pinduoduo|pdd|1688|xiaohongshu|douyin|kuaishou|weidian|youzan)\.|"  # 常见电商域名
    r"(?:淘宝|天猫|拼多多|京东|微店|有赞|小店|下单|优惠券|领券|加微|加v|扫码|店铺|代购|接单)",   # 中文店铺/引流词
    re.IGNORECASE,
)
...
class CardMonitorMixin:
    ...
    async def _card_llm_violation(self, group_id: str, user_id: str, card: str, hit_types: dict,
        ...
        """LLM 上下文判断名片是否违规（与消息审核同款流程）。LLM 关闭/失败时回退为"按初筛命中即违规"。

        allow_promo=True 时：允许引流/联系方式暗示（如 xxxrxxx），仅当含明确链接/二维码/店铺时才判违规。
        """
        if not self._cfg("llm_moderation_enabled", True, group_id=group_id):
            return True  # 未开 LLM：初筛命中即判违规
        ...
        if allow_promo:
            ...
            rule = ("识别引流、店铺、联系方式、网址链接、辱骂、色情、违禁内容，这些都算违规。"
                    "正常昵称/自我介绍不算违规。")
        prompt = (
            "判断下面这个群名片是否需要还原（是否违规）。名片是短文本，无需过度联想。\n"
            f"【判定规则】{rule}\n"
            f"【群名片】<<<{card}>>>\n"
            f"【初筛可疑类型】{suspect}\n"
        ...
        return True  # LLM 失败：保守起见按初筛命中即违规（初筛已命中才会调到这里）
    ...
    async def _handle_group_card_change(self, event: AstrMessageEvent) -> bool:
        ...
        # C 违规名片审核。两种模式独立开关：
        #   - card_audit_link_only：只拦链接/二维码/店铺，允许引流缩写等自定义昵称（宽松）
        #   - card_audit_enabled：全量违规审核（引流/辱骂/违禁词等都拦），可配 card_audit_llm_always
        #     强制对每个名片走 LLM，识别 ldxp/catfk 这类无规律引流缩写
        # 两者可同时开；link_only 命中即还原，不再走全量流程。还原目标默认旧名片，旧名片也脏则清空。
        ...
        if not restored and (link_only or full_audit):
            is_violation = False
            reason = ""
            has_link = self._is_shop_link_card(card_new)
            if has_link:
                is_violation = True
                reason = "含链接/店铺"
            elif full_audit:
                ...
                allow_promo = link_only  # link_only 同时开时，全量判断也放行引流
                if hit_types or force_llm:
                    is_violation = await self._card_llm_violation(
                        group_id, user_id, card_new, hit_types, allow_promo=allow_promo)
