...
- OneBot 响应支持多层 `data`/`payload` 包装，以及 `group_list` / `groupList` / `group_members` 等协议字段。
- 失败包、畸形行和空列表不再把已有缓存写成“成功但没群”；无缓存时返回明确错误。
- 部分协议端拒绝 `no_cache` 时自动去掉该参数重试。

### 修复：审核日志不能翻页（Issue #71）

- `/logs` 改为分页契约 `{items, total, page, page_size}`，与词库/规则页一致。
- WebUI 审核日志支持上一页/下一页、每页 20/50/100 和跳页，去掉前端 `slice(0, 100)` 截断。

### 修复：申诉提示刷屏与次数

- 非文字私聊只提示一次“请用文字说明你的申诉理由”。
- 文字申诉最多两次；机器人自己的提示回显不再消耗次数。

### 其他

- 识别方框字/带圈字/同形字母/零宽字符伪装的链接，随广告检测开关控制。
- 嵌套重复投递不再额外计入刷屏速率；名片同步兼容旧协议响应和失败包。
- 管理员内容审核豁免改为可选，默认关闭。
