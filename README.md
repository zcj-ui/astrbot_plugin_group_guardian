...
> 当前版本：**v2.8.4** | 最低 AstrBot 版本：**>=4.24.2**
...
| 异形字符伪装链接 | 直接拦截 | 方框字、带圈字、同形外文字母、零宽字符或逐字空格伪装的 HTTP/HTTPS 链接 |
...
- 管理员内容审核豁免可选，默认管理员也会参与内容审核；防刷屏始终豁免管理员
- 群白名单/黑名单机制
- 用户黑名单（自动拒绝加群）
...
- **异形链接识别**：识别 QQ 方框/带圈拉丁字母、常见同形外文字母、全角标点、零宽字符和逐字空格伪装的 HTTP/HTTPS 链接；命中后保留原文并记录归一化证据，随广告检测开关控制
- **OCR识图审核**：使用LLM视觉模型识别图片内容，支持3种内置提示词模板（通用识别/严格审核/纯文字转录）和自定义提示词
- **图片审核前提**：图片中的文字或二维码只有启用 `ocr_enabled`、`qrcode_decode_enabled` 或 `llm_moderation_always` 后才会进入语义审核；`ocr_enabled` 默认关闭，以避免未经配置的视觉模型调用
