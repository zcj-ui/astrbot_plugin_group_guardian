...
- **防刷屏检测（v2.2.1+）**：追踪所有消息类型（转发/QQ收藏/图片/JSON等）的发送频率，支持每秒/每分钟/每小时独立速率限制（每档可设 0 关闭），支持凌晨/夜间时段单独阈值，超限自动禁言+可选批量撤回，撤回阈值可自定义。WebUI 专属刷屏监控面板实时展示每群每人的消息速率，支持红/黄/绿预警状态
