...
> 当前版本：**v2.33.0** | 最低 AstrBot 版本：**>=4.24.2**
...
WebUI 面板的**远程群管操作**（禁言/踢人/设名片等）遵循以下权限模型：

**1. Dashboard 用户与 QQ 身份绑定（`web_operator_bindings`）**
- WebUI 远程操作时前端携带**操作者用户名**（AstrBot Dashboard 登录用户名，本地记忆）；服务端按
  `web_operator_bindings`（格式 `用户名:QQ号,用户名2:QQ2`）**服务端绑定**映射到 **QQ 身份**，用于授权校验与审计；
...
- 开启后：远程写操作**必须**校验操作者授权，未绑定身份或非授权用户一律拒绝；
...
- **视频广告检测（v2.9.0+）**：自动下载群内视频（受体积/超时上限保护），用 OpenCV 等间隔抽帧，逐帧走视觉模型识别 + 二维码解码，识别文本并入正文走统一「初筛 + LLM 二次判断 + 处罚」流程；识别失败静默降级不误杀（默认关闭，可按群覆盖）。配置与验证详见 [`视频广告识别-使用说明.md`](视频广告识别-使用说明.md)
...
- **广告后台（v2.21.0 起并入 AstrBot Dashboard 主面板）**：查看广告拦截统计、违规记录（含图片证据）、感知哈希黑名单、视频指纹缓存、分级处置记录与配置状态，支持删除黑名单/清空指纹/重置分级与视频复核等管理操作；v2.10.x 的独立 HTTP 后台（默认端口 8765）与独立页面已移除（v2.31.0 删除遗留的 `pages/ad_backend/index.html` 页面入口，统一在主面板「广告后台」页签操作）
- **不确定内容私信管理员复核（v2.32.0+）**：审核遇到「不确定」内容（视频广告疑似、文本/图片 LLM 输出 `unknown`/无法确认）时，可**立即私信该群全部管理员**（群主+管理员）发送内容与编号供重新审核，管理员直接私聊回复「确认复核 #编号 / 放行复核 #编号」完成复核；视频广告私信用「确认广告 / 放行广告」；全部默认关闭（`uncertain_review_enabled` / `uncertain_review_private_admin` / `video_ad_review_private_admin`）
- **发送安全壳（v2.33.0+）**：给 `event.send` 加安全壳，NapCat/OneBot 发送动作超时（retcode=1200）等失败不再被 AstrBot RespondStage 重新抛出打崩整个进程，仅记日志丢消息不崩溃（`safe_send_enabled`，默认开，可按群覆盖）
...
| 广告后台（v2.21.0 起接入 Dashboard 主面板，v2.31.0 移除独立页面入口） + 本地 RapidOCR + 多识别引擎（local/umi/cloud/llm/auto） | v2.10.x |
| 不确定内容私信该群全部管理员重新审核（文本/图片 LLM unknown 三态 + 视频广告私信复核） | v2.32.0 |
